...
* The new checkpoint will do a reflection on previous checkpoints, prior mutations, accepted fixes, rejected changes, and failure summaries before generating the next candidate
...
Define the optimization hyper parameters explicitly:

* prompt itself: the current prompt candidate being evaluated
* baseline prompt: the current active prompt used as the comparison reference
* temperature: handles LLM creativity and response randomness
...
1. `candidate_score >= baseline_score + min_delta`
   ...
   `baseline_score`: the score of the current active prompt (the one being compared against)
...
* stop when the candidate score reaches the configured target threshold
...
* defined hyper parameters, including baseline prompt and temperature
