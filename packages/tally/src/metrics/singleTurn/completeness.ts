...
export function createCompletenessMetric<TContainer extends DatasetItem | ConversationStep>(
	...
	const formattedExpectedPoints = expectedPoints && expectedPoints.length > 0
...
Given the query and response below, analyze the response's completeness using the provided rubric.${formattedExpectedPoints ? `\n\nExpected topics/points that should be covered:\n${formattedExpectedPoints}` : ''}
