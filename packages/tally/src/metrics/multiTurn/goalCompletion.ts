...
Given the goal and the conversation below, analyze the assistant's goal completion using the provided rubric.${checkPartialCompletion ? '\n\nPay attention to partial completion - evaluate whether the assistant makes meaningful progress toward the goal even if not fully achieved.' : ''}${considerEfficiency ? '\n\nConsider efficiency - evaluate how many turns it took to achieve the goal.' : ''}
...
2. Quality of the solution or approach taken${checkPartialCompletion ? '\n3. Progress made toward the goal (even if not fully achieved)' : ''}${considerEfficiency ? '\n4. Efficiency in achieving the goal (fewer turns is better)' : ''}`,
			scale: '0-5 scale where 5 = fully achieves goal efficiently, 0 = no progress toward goal',
			examples: [
				{
					score: 5,
					reasoning: 'Assistant fully achieves the goal efficiently and effectively with a high-quality solution',
				},
				{
					score: 4,
