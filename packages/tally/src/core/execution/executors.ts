...
import { type GenerateObjectOptions, generateMetricValue } from './llm/generateObject';
...
class LLMMetricExecutor<TTarget, T extends MetricScalar> implements MetricExecutor<TTarget, T> {
  async execute(
    metricDef: MetricDef<T, MetricContainer>,
    target: TTarget,
    options?: ExecutorOptions
    ...
    if (scope === 'single') {
      ...
      context = buildSingleTurnLLMContext(target as SingleTargetFor<unknown>);
    } else {
      context = buildMultiTurnLLMContext(target as unknown as Conversation);
    }
...
class CodeMetricExecutor<TTarget, T extends MetricScalar> implements MetricExecutor<TTarget, T> {
  async execute(
    metricDef: MetricDef<T, MetricContainer>,
    target: TTarget,
    options?: ExecutorOptions
    ...
    const cacheable = possibleCode.type === 'code-based' && possibleCode.cacheable !== false;
    ...
    if (scope === 'single') {
      ...
      const meta = isDatasetItem(target)
        ? (target as unknown as DatasetItem).metadata
        : isConversationStep(target)
          ? (target as unknown as ConversationStep).metadata
          : undefined;
      const computeArgs =
        meta !== undefined
...
function buildMultiTurnLLMContext(conversation: Conversation): Record<string, unknown> {
  const context: Record<string, unknown> = {
