...
export async function runMultiTurnMetric<T extends MetricScalar>(
	...
	const resolvedPreprocessor: PreprocessorFn =
		preprocessFields.preprocessContainer
			? ((c) => {
					const fn = preprocessFields.preprocessContainer as (container: Conversation) => Promise<unknown> | unknown;
					return fn(c);
				})
			: preprocessFields.runOnContainer
				? ((c) => {
						const fn = preprocessFields.runOnContainer as (container: Conversation) => Promise<unknown> | unknown;
						return fn(c);
					})
...
export async function runMultiTurnMetrics<T extends MetricScalar>(
	...
	const results = await Promise.all(
		conversations.map((conversation) =>
			runMultiTurnMetric(metricDef, conversation, options)
		)
	);
