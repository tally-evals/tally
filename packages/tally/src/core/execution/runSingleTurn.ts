...
export async function runSingleTurnMetric<T extends MetricScalar, TContainer>(
	...
	type PreprocessorFn = (selected: SingleTargetFor<unknown>, metric: unknown) => Promise<unknown> | unknown;
	const resolvedPreprocessor: PreprocessorFn =
		preprocessFields.preProcessor
			? ((s) => {
					const fn = preprocessFields.preProcessor as (selected: SingleTargetFor<unknown>) => Promise<unknown> | unknown;
					return fn(s);
				})
