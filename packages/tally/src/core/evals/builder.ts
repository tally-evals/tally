...
export function buildFromEvals<TContainer extends MetricContainer>(
	...
	const evalMetadata = new Map<string, { evalKind: 'singleTurn' | 'multiTurn' | 'scorer'; verdictPolicy?: VerdictPolicy }>();

	// Check for duplicate eval names
	const evalNames = new Set<string>();
	for (const eval_ of evals) {
		if (evalNames.has(eval_.name)) {
			throw new Error(`Duplicate eval name: "${eval_.name}". Eval names must be unique.`);
		}
...
function buildSingleTurnEval<TContainer extends SingleTurnContainer>(
	...
	if (needsAutoNormalization(metric)) {
		const autoNormalizer = eval_.autoNormalize ?? getDefaultAutoNormalizer(detectMetricValueType(metric));
		if (autoNormalizer) {
