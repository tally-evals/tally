...
export function normalizeZScore(
	...
	const mean = spec.mean ?? context.distribution?.mean ?? context.extra?.mean as number | undefined;
	const stdDev = spec.stdDev ?? context.distribution?.stdDev ?? context.extra?.stdDev as number | undefined;

	if (mean === undefined || stdDev === undefined) {
		throw new Error(
			'Z-score normalizer requires mean and stdDev in spec or context.distribution'
		);
	}
