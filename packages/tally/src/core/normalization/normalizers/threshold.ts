...
export function normalizeThreshold(
	...
	const threshold = spec.threshold ?? context.thresholds?.pass ?? context.extra?.threshold as number | undefined;

	if (threshold === undefined) {
		throw new Error(
			'Threshold normalizer requires threshold in spec or context.thresholds.pass'
		);
	}
