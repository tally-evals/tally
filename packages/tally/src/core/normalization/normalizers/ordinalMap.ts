...
import type { Score, ScoringContext, MetricScalar, NormalizerSpec } from '@tally/core/types';
...
export function normalizeOrdinalMap(
	...
	const map = spec.map ?? context.ordinalMap ?? context.extra?.map as Record<string | number, number> | undefined;

	if (!map) {
		throw new Error(
			'Ordinal map normalizer requires map in spec or context.ordinalMap'
		);
	}
	...
	if (mappedValue < 0 || mappedValue > 1) {
		throw new Error(
			`Ordinal map normalizer: mapped value ${mappedValue} must be in [0, 1] range`
		);
	}
