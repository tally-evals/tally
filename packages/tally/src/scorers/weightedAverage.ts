...
function computeWeightedAverage<TInputs extends readonly ScorerInput[]>(
	...
	for (const input of inputs) {
		if (input.required !== false) {
			...
			if (score === undefined) {
				throw new Error(
					`Required metric "${metricName}" is missing from scores`
				);
			}
	...
	for (const input of inputs) {
		...
		if (typeof score !== 'number' || score < 0 || score > 1) {
			throw new Error(
				`Score for metric "${metricName}" must be in [0, 1] range, got ${score}`
			);
		}

		const weight = normalizeWeights
			? Math.abs(input.weight) / totalWeight
