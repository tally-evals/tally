...
export function formatReportAsTables(
	...
	for (let i = 0; i < report.perTargetResults.length; i++) {
		...
		formatConversationTable(result, conversation, i + 1);
	}
...
function formatConversationTable(
		...
		for (const evalName of singleTurnEvalNames) {
			...
			if (stepIdx < metrics.length) {
				...
				if (metric) {
					...
					const verdictIcon = verdict ? (verdict.verdict === 'pass' ? '✓' : verdict.verdict === 'fail' ? '✗' : '?') : '';
	...
	for (const metric of multiTurnMetrics) {
		const evalName = metric.metricDef.name;
		if (seenMultiTurnMetrics.has(evalName)) continue;
		seenMultiTurnMetrics.add(evalName);
