...
export function isMetricDef(value: unknown): value is MetricDef {
  ...
  return (
    ...
    (def.scope === undefined || def.scope === 'single' || def.scope === 'multi')
  );
}
...
export function assertDatasetItem(value: unknown, message?: string): asserts value is DatasetItem {
  if (!isDatasetItem(value)) {
    throw new Error(message || `Expected DatasetItem, got ${typeof value}`);
  }
}
...
export function assertConversation(
  ...
  if (!isConversation(value)) {
    throw new Error(message || `Expected Conversation, got ${typeof value}`);
  }
}
 ...
 */
export function assertScore(value: unknown, message?: string): asserts value is Score {
  if (!isScore(value)) {
    throw new Error(message || `Expected Score in [0, 1] range, got ${value}`);
  }
}
