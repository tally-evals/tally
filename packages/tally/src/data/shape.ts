 ...
 */
export function adaptToDataset(items: unknown[], options?: ShapeAdapterOptions): DatasetItem[] {
...
export function adaptToConversationStep(
  ...
  if (typeof step !== 'object' || step === null) {
    throw new Error(`Cannot adapt non-object to ConversationStep at index ${stepIndex}`);
  }
  ...
  if (!isModelMessage(input)) {
    throw new Error(`Missing or invalid "input" field at step ${stepIndex}. Expected ModelMessage`);
  }
...
export function adaptToConversation(
  ...
  if (!Array.isArray(stepsSource)) {
    throw new Error(`Missing or invalid "steps" field at index ${index}. Expected array`);
  }
  ...
  const metadata =
    source.metadata !== undefined ? (source.metadata as Record<string, unknown>) : undefined;
...
export function adaptToConversations(
  ...
  return conversations.map((conv, index) => adaptToConversation(conv, index, options));
}
