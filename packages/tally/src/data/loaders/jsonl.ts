...
import { isValidDatasetItem, isValidConversation, isValidConversationStep } from '../validate';
import type { ShapeAdapterOptions } from '../shape';
import { adaptToDatasetItem, adaptToConversation, adaptToConversationStep } from '../shape';
...
export async function loadConversationStepsFromJSONL(
			...
			try {
				...
				if (validate) {
					if (!isValidConversationStep(step)) {
						const error = new Error(
							`Invalid ConversationStep at line ${lineNumber}`
						);
						if (skipInvalid) {
							console.warn(error.message);
							continue;
	...
	if (steps.length === 0) {
		throw new Error(
			`No steps found for conversation ${conversationId} in fixture: ${filePath}`
		);
	}
