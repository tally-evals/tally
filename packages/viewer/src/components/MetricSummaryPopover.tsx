...
export function MetricSummaryPopover(props: {
  ...
  const scoreAggs = (content.summary?.aggregations?.score ?? undefined) as
    | Record<string, unknown>
    | undefined;
  const rawAggs = (content.summary?.aggregations?.raw ?? undefined) as
    | Record<string, unknown>
    | undefined;
            ...
            <div className="space-y-1">
              ...
              <div className="rounded-md border border-border bg-muted/20 p-2">
                ...
                <div className="text-xs">
                  ...
                  <span className="font-mono">{content.scorerDef?.combine?.kind ?? 'unknown'}</span>
                </div>
                <div className="text-xs">
            ...
            <div className="rounded-md border border-border bg-muted/20 p-2 text-xs">
              {showVerdict && verdictSummary ? (
                <span className="font-mono">
                  ...
                  <span className="text-red-600 dark:text-red-400">{verdictSummary.failCount}</span>{' '}
                  <span className="text-muted-foreground">·</span>{' '}
