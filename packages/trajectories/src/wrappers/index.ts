 ...
 */
export function withAISdkAgent(agent: {
 ...
 * @returns AgentHandle that can be used with trajectories
 */
export function withAISdkAgent(config: Omit<GenerateTextInput, 'messages' | 'prompt'>): AgentHandle;
      ...
      async respond(agentMemoryMessages: readonly ModelMessage[]) {
        ...
        try {
          modelMessages = convertToModelMessages(
            agentMemoryMessages as unknown as never
        ...
        const rawOut = result.response.messages as unknown as Array<{
          role?: unknown;
          content?: unknown;
        }>;
        ...
        if (hasPartsContent) {
          try {
            const converted = convertToModelMessages(
              result.response.messages as unknown as never
            ) as unknown as ModelMessage[];
 ...
 * @returns AgentHandle that can be used with trajectories
 */
export function withMastraAgent(agent: {
