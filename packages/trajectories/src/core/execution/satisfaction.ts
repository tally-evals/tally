...
import type { SatisfactionContext, StepDefinition, StepRuntimeState } from '../steps/types.js';
...
function defaultSatisfactionHeuristic(
  ...
  const mentionsKeyword =
    contentStr.length > 0 && instructionHints.some((kw) => contentStr.toLowerCase().includes(kw));
