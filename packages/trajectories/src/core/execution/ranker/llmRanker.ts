...
- topCandidates: up to 3 items, sorted by confidence desc, each with { stepIndex, confidence, reasoning }`;
  ...
  try {
    const result = await generateObject({
      model,
      system:
        "You are analyzing an ongoing conversation to select the next best user step. Consider both the user's latest message and the assistant's response to determine what step the user should take next.",
      prompt,
