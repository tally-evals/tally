...
import { logStep, logTrajectoryEnd, logTrajectoryStart } from '../utils/logger.js';
import { invokeAgent } from './execution/agentInvoker.js';
import { analyzeAgentLoopFromStepIds } from './execution/loopDetector.js';
import { buildPolicyContext, createPolicy, evaluatePolicy } from './execution/policyEvaluator.js';
...
import type { StepId, StepRuntimeState } from './steps/types.js';
import type { AgentHandle, StepTrace, Trajectory, TrajectoryResult } from './types.js';
...
export async function runTrajectory(
  ...
  if (generateLogs) {
    logTrajectoryStart(trajectory.goal, trajectory.persona, conversationId);
  }
  ...
  while (true) {
    ...
    // Evaluate policy
    const policyContext = buildPolicyContext(trajectory, steps, currentStepId, stepToUse);
    ...
    if (policyResult.shouldStop) {
      ...
      const summary = policyResult.message !== undefined ? policyResult.message : undefined;
    ...
    if (chosenStepId && stepToUse) {
      ...
      const satisfied = await evaluateSatisfaction(stepToUse, newState, steps, {
        satisfied: new Set(
