...
const travelPlannerTools = {
  searchFlights: tool({
    ...
    inputSchema: z.object({
      ...
      destination: z
        .string()
        .describe('Arrival city and airport code (e.g., "San Francisco, SFO")'),
  ...
  describe('Travel Planner - Step Progression', () => {
    it('should progress through steps sequentially without jumping to terminal', async () => {
      ...
      const hasFlightSearch =
        toolCallCounts.has('searchFlights') && (toolCallCounts.get('searchFlights') ?? 0) > 0;
      const hasAccommodationSearch =
        toolCallCounts.has('searchAccommodations') &&
        (toolCallCounts.get('searchAccommodations') ?? 0) > 0;
      const hasWeatherSearch =
        toolCallCounts.has('getWeatherForecast') &&
        (toolCallCounts.get('getWeatherForecast') ?? 0) > 0;

      console.log('\n🔍 Tool Usage:');
      console.log(
        `  Flight search: ${hasFlightSearch ? '✅' : '❌'} (${toolCallCounts.get('searchFlights') ?? 0} calls)`
      );
      console.log(
        `  Accommodation search: ${hasAccommodationSearch ? '✅' : '❌'} (${toolCallCounts.get('searchAccommodations') ?? 0} calls)`
      );
      console.log(
        `  Weather search: ${hasWeatherSearch ? '✅' : '❌'} (${toolCallCounts.get('getWeatherForecast') ?? 0} calls)`
      );
