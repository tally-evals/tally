...
import { hasApiKey, shouldRunE2E } from './setup.js';
...
const weatherAgent = new Agent({
  ...
  system:
    'You are a helpful weather assistant. Help users get weather information by asking for locations when needed and using the weather tool.',
});
  ...
  describe('withAISdkAgent - Agent Instance Pattern', () => {
    it('should run trajectory with AI SDK Agent instance', async () => {
      ...
      const trajectory = createTrajectory(
        {
          ...
          persona: {
            ...
            description:
              'You need weather information for different locations. When asking for weather, always include both the city and state (e.g., "San Francisco, CA" or "New York, NY"). Ask clearly and provide location names accurately.',
            guardrails: [
              'Ask naturally',
              'Always include both city and state when asking for weather',
      ...
      for (const step of result.steps) {
        ...
        expect(
          step.agentMessages.every((msg) => msg.role === 'assistant' || msg.role === 'tool')
        ).toBe(true);
      ...
      const trajectory = createTrajectory(
        {
          ...
          persona: {
            description:
              'You need weather information. When asking for weather, always include both the city and state (e.g., "San Francisco, CA").',
          },
          steps: {
      ...
      const trajectory = createTrajectory(
        {
          ...
          persona: {
            description:
              'You need weather information for different locations. When asking for weather, always include both the city and state (e.g., "San Francisco, CA").',
          },
          steps: {
            steps: [{ id: 'step-0', instruction: 'Ask for current weather in San Francisco, CA' }],
            start: 'step-0',
      ...
      const trajectory = createTrajectory(
        {
          ...
          persona: {
            description:
              'You need weather information. When asking for weather, always include both the city and state (e.g., "San Francisco, CA" or "New York, NY").',
          },
          steps: {
            steps: [
              { id: 'step-0', instruction: 'Ask for current weather in San Francisco, CA' },
