...
export function extractToolCallsFromStep(step: ConversationStep): ExtractedToolCall[] {
  ...
  for (const message of step.output) {
        ...
        for (const part of message.content) {
          if (
            ...
            toolCalls.push({
              ...
              input:
                'input' in partObj ? partObj.input : 'args' in partObj ? partObj.args : undefined,
            });
          }
        }
      }
    ...
    if (message.role === 'tool' && 'toolCallId' in message) {
      ...
      const _toolName = 'toolName' in message ? String(message.toolName) : undefined;
 ...
 */
export function countToolCallsByType(conversation: { steps: readonly ConversationStep[] }): Map<
  string,
  number
> {
