---
title: Reports
description: SDK report objects, View API, and the schema-stable run artifact.
---
...
<PropertyTable
  content={[
    { name: 'stepCount', type: 'number', description: 'Total number of steps in the conversation.' },
    { name: 'defs', type: 'RunDefs', description: 'All definitions (metrics, evals, scorers) used in the run.' },
    { name: 'step(index)', type: '(index: number) => StepResults', description: 'Get all single-turn eval results for a specific step.' },
    { name: 'steps()', type: '() => Generator<StepResultsWithIndex>', description: 'Iterate over all steps with their results.' },
    { name: 'conversation()', type: '() => ConversationResults', description: 'Get multi-turn evals and scalar scorer results.' },
    { name: 'summary()', type: '() => SummaryResults | undefined', description: 'Get aggregated summaries by eval name.' },
    { name: 'metric(name)', type: '(name: string) => MetricDefSnap | undefined', description: 'Look up a metric definition by name.' },
    { name: 'eval(name)', type: '(name: string) => EvalDefSnap | undefined', description: 'Look up an eval definition by name.' },
    { name: 'scorer(name)', type: '(name: string) => ScorerDefSnap | undefined', description: 'Look up a scorer definition by name.' },
    { name: 'metricForEval(name)', type: '(name: string) => MetricDefSnap | undefined', description: 'Get the metric definition for an eval.' },
  ]}
/>
  ...
  content={[
    {
      name: '[evalName]',
      type: 'StepEvalResult',
      description: 'Result for a single-turn eval at this step. Keys are literal eval names.',
    },
  ]}
/>
...
<PropertyTable
  content={[
    { name: 'index', type: 'number', description: 'Step index (0-based).' },
    {
      name: '[evalName]',
      type: 'StepEvalResult',
      description: 'Result for a single-turn eval at this step.',
    },
  ]}
/>
  ...
  content={[
    {
      ...
      description: 'Result for a multi-turn eval or scalar scorer. Keys are literal eval names.',
    },
  ]}
/>
  ...
  content={[
    {
      name: '[evalName]',
      type: 'EvalSummary',
      description: 'Summary for an eval including aggregations and verdict rates.',
    },
  ]}
/>
...
<PropertyTable
  content={[
    { name: 'evalRef', type: 'string', description: 'Name of the eval.' },
    { name: 'measurement', type: 'Measurement', description: 'The measurement (score, rawValue, metadata).' },
    { name: 'outcome', type: 'EvalOutcome', description: 'Verdict outcome (if verdict policy was defined).', isOptional: true },
  ]}
/>
...
<PropertyTable
  content={[
    { name: 'metricRef', type: 'string', description: 'Reference to the metric in defs.metrics.' },
    { name: 'score', type: 'number', description: 'Normalized score (0-1) after normalization.', isOptional: true },
    { name: 'rawValue', type: 'number | boolean | string | null', description: 'Original metric value before normalization.', isOptional: true },
    { name: 'confidence', type: 'number', description: 'LLM confidence (0-1) if applicable.', isOptional: true },
    { name: 'reasoning', type: 'string', description: 'LLM reasoning if applicable.', isOptional: true },
    { name: 'executionTimeMs', type: 'number', description: 'Execution time in milliseconds.', isOptional: true },
    { name: 'timestamp', type: 'string', description: 'ISO timestamp of measurement.', isOptional: true },
    { name: 'metadata', type: 'Record<string, unknown>', description: 'Additional metadata.', isOptional: true },
  ]}
/>
...
<PropertyTable
  content={[
    { name: 'verdict', type: "'pass' | 'fail' | 'unknown'", description: 'The final verdict.' },
    { name: 'policy', type: 'VerdictPolicyInfo', description: 'The policy used to compute the verdict.' },
    { name: 'observed', type: '{ rawValue?: MetricScalar; score?: number }', description: 'The observed values used for the verdict.', isOptional: true },
  ]}
/>
...
<PropertyTable
  content={[
    { name: 'eval', type: 'string', description: 'Eval name.' },
    { name: 'kind', type: "'singleTurn' | 'multiTurn' | 'scorer'", description: 'Eval kind.' },
    { name: 'count', type: 'number', description: 'Number of data points.' },
    {
      name: 'aggregations',
      type: '{ score: Record<string, number>; raw?: Record<string, number | Record<string, number>> }',
      description: 'Aggregated statistics (mean, percentiles, etc.).',
      isOptional: true,
    },
    { name: 'verdictSummary', type: 'VerdictSummary', description: 'Pass/fail/unknown rates and counts.', isOptional: true },
  ]}
/>
...
<PropertyTable
  content={[
    { name: 'passRate', type: 'number', description: 'Proportion of passing verdicts (0-1).' },
    { name: 'failRate', type: 'number', description: 'Proportion of failing verdicts (0-1).' },
    { name: 'unknownRate', type: 'number', description: 'Proportion of unknown verdicts (0-1).' },
    { name: 'passCount', type: 'number', description: 'Number of passing verdicts.' },
    { name: 'failCount', type: 'number', description: 'Number of failing verdicts.' },
    { name: 'unknownCount', type: 'number', description: 'Number of unknown verdicts.' },
    { name: 'totalCount', type: 'number', description: 'Total number of verdicts.' },
  ]}
/>
