...
```ts
import { createSingleTurnCode, defineBaseMetric, createPercentileAggregator } from 'tally';

const latency = createSingleTurnCode({
...
- **Compatibility**: use `CompatibleAggregator<TMetricValue>` to ensure you only attach aggregators that make sense for the metric’s raw value type.
