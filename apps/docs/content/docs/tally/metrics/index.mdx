...
4.  **Aggregate (Optional)**: For single-turn metrics, attach [Aggregators](/docs/tally/reference/aggregators) to compute summary statistics (mean, percentiles, etc.) across all targets. Tally provides sensible defaults based on metric value type.
5.  **Compose**: Use [Scorers](/docs/tally/scorers) to combine multiple signals, then use [Evals](/docs/tally/evals) to apply a verdict policy.
...
See the full reference: [Normalizers](/docs/tally/reference/normalizers).
