...
```ts
import { createSingleTurnCode, createMultiTurnLLM, defineBaseMetric } from '@tally-evals/tally';

// Single-turn: "Does this step include a tool call?"
const hasToolCall = createSingleTurnCode({
...
const stayedOnTopic = createMultiTurnLLM({
  base: defineBaseMetric({
    name: 'stayedOnTopic',
    valueType: 'number',
  }),
...
```ts
import { createSingleTurnCode, createSingleTurnLLM, defineBaseMetric } from '@tally-evals/tally';
import { createMinMaxNormalizer } from '@tally-evals/tally/normalization';

// Code-based: optional preProcessor, cacheable, metadata
const keywordPresent = createSingleTurnCode({
  base: defineBaseMetric({
    name: 'keywordPresent',
    valueType: 'boolean',
  }),
...
const helpfulness = createSingleTurnLLM({
  base: defineBaseMetric({
    name: 'helpfulness',
    valueType: 'number',
  }),
...
- [Metric Factories](/docs/tally/reference/metric) — `defineBaseMetric`, `createSingleTurnCode`, `createSingleTurnLLM`, `withNormalization`
