...
```ts
const failures = report.perTargetResults.filter(r => r.verdicts.some(v => v.status === 'fail'));
...
```ts
const report = await tally.run();
const relevance = report.evalSummaries.find(s => s.evalName === 'Answer Relevance');

if (relevance.passRate < 0.95) {
  throw new Error(`Evaluation Failed: Pass rate (${relevance.passRate}) is below 95%`);
}
```
