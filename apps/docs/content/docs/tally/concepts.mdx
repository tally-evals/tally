...
Tally is primarily an **Evaluation Engine**. Instead of providing a rigid set of fixed tests, it provides the infrastructure to define and run your own domain-based evaluation logic. 
...
Tally is composed of several fundamental building blocks that you wire together to form an evaluation pipeline.
...
### 3. Evals (Decision)
Focuses exclusively on the business rules. An eval takes the output of your logic and applies a **Verdict Policy**. You can use the same scoring logic but apply a strict verdict for production and a loose one for experiments.

## Core Architecture

Tally acts as the orchestrator that wires these building blocks together against your data.
