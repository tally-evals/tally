...
export const metricDefExample = String.raw`import { defineSingleTurnCode, defineBaseMetric } from '@tally-evals/tally'

const length = defineBaseMetric({ name: 'answerLength', valueType: 'number' })

const answerLength = defineSingleTurnCode({
  base: length,
  compute: ({ selected }) => selected.output[0]?.content.length ?? 0,
})`;
...
export const tallyTsDef = String.raw`export interface Tally<
...
> {
  ...
  run(options?: RunOptions): Promise<TallyRunReport<TEvals>>;
}
`;

export const tallyExample = String.raw`import { createTally } from '@tally-evals/tally'

const tally = createTally({
  data: [conversation],
  evals: [relevanceEval, completenessEval],
})

const report = await tally.run()`;

export const reportTsDef = String.raw`export interface TallyRunReport<TEvals extends readonly Eval[]> {
...
view.step(0)        // Single-turn results for step 0
view.conversation() // Multi-turn and scorer results
view.summary()      // Aggregated summaries`;
...
| [**Tally**](/docs/tally/getting-started) | **Orchestrator** | The main entry point that takes data and evals, then runs the evaluation pipeline. | <CodePopover title="Tally (core)" tsDef={tallyTsDef} example={tallyExample} /> |
| [**Report**](/docs/tally/reports) | **Scorecard** | The type-safe output with per-target results, summaries, and the View API for assertions. | <CodePopover title="TallyRunReport (core)" tsDef={reportTsDef} example={reportExample} /> |
...
- [Metrics API](/docs/tally/reference/metric) — `defineBaseMetric`, `defineSingleTurnCode`, `defineSingleTurnLLM`, etc.
...
- [Reports API](/docs/tally/reference/reports) — `TallyRunReport`, `TallyRunArtifact`, View API
