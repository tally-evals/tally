...
import {
  type Dining,
  diningSchema,
  searchDiningParamsSchema,
} from '~/schemas/travel-planner/dining';
...
function generateDiningName(seededFaker: typeof faker, cuisine: string, type: string): string {
  ...
  const middle =
    seededFaker.helpers.maybe(() => seededFaker.person.lastName(), { probability: 0.3 }) || '';
...
export const searchDiningTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    for (let i = 0; i < MIN_GENERATED_DINING; i++) {
      ...
      const dressCode =
        diningType === 'fine dining'
          ? 'formal'
          : diningType === 'food truck'
            ? 'no dress code'
            : faker.helpers.arrayElement(['casual', 'smart casual']);

      // Reservations
      const reservationsAccepted = diningType !== 'food truck';
      const reservationsRequired =
        diningType === 'fine dining' || (rating > 4.5 && faker.datatype.boolean(0.6));
    ...
    if (ambiance) {
      filteredDining = filteredDining.filter(
        (d) => d.ambiance.toLowerCase() === ambiance.toLowerCase()
      );
    }
