...
import {
  ...
  vehicleSchema,
} from '~/schemas/travel-planner/vehicle';
...
function getMakeForType(seededFaker: typeof faker, vehicleType: string): string {
  const validMakes = Object.entries(VEHICLE_CATALOG)
    .filter(([_, info]) => isVehicleType(vehicleType) && info.types.includes(vehicleType))
    .map(([make]) => make);

  return validMakes.length > 0 ? seededFaker.helpers.arrayElement(validMakes) : 'Toyota';
}
...
function generateSpecs(
  ...
  const horsepower =
    vehicleType === 'sports car'
      ? seededFaker.number.int({ min: 400, max: 700 })
      : vehicleType === 'luxury'
        ? seededFaker.number.int({ min: 300, max: 500 })
        : vehicleType === 'compact'
          ? seededFaker.number.int({ min: 120, max: 180 })
  ...
  const mpg =
    fuelType === 'electric'
      ? 'N/A (Electric)'
      : fuelType === 'hybrid'
        ? `${seededFaker.number.int({ min: 40, max: 60 })} MPG combined`
        : `${seededFaker.number.int({ min: 20, max: 35 })} MPG combined`;

  const acceleration =
    vehicleType === 'sports car'
      ? `${seededFaker.number.float({ min: 2.5, max: 4.5 }).toFixed(1)}s`
      : vehicleType === 'luxury'
        ? `${seededFaker.number.float({ min: 4.0, max: 6.0 }).toFixed(1)}s`
        : `${seededFaker.number.float({ min: 6.0, max: 10.0 }).toFixed(1)}s`;

  const topSpeed =
    vehicleType === 'sports car'
      ? `${seededFaker.number.int({ min: 180, max: 220 })} mph`
      : vehicleType === 'luxury'
        ? `${seededFaker.number.int({ min: 140, max: 180 })} mph`
  ...
  const engine =
    fuelType === 'electric'
      ? 'Electric Motor'
      : fuelType === 'hybrid'
        ? `${seededFaker.number.float({ min: 1.5, max: 2.5 }).toFixed(1)}L Hybrid`
  ...
  execute: async ({ context }) => {
    ...
    for (let i = 0; i < MIN_GENERATED_VEHICLES; i++) {
      ...
      if (make) {
        ...
        vehicleMake =
          isVehicleType(vehicleType) && catalog?.types.includes(vehicleType)
            ? make
            : getMakeForType(faker, vehicleType);
      } else {
