...
import {
  type Accommodation,
  accommodationSchema,
  searchAccommodationsParamsSchema,
} from '~/schemas/travel-planner/accommodations';
...
function generateAccommodationName(seededFaker: typeof faker, type: string): string {
  const prefix = seededFaker.helpers.arrayElement(
    NAME_PREFIXES[type as keyof typeof NAME_PREFIXES]
  );
  const suffix = seededFaker.helpers.arrayElement(
    NAME_SUFFIXES[type as keyof typeof NAME_SUFFIXES]
  );
...
export const searchAccommodationsTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    for (let i = 0; i < MIN_GENERATED_ACCOMMODATIONS; i++) {
      ...
      const airport =
        Math.random() > 0.3
          ? Number(faker.number.float({ min: 5, max: 30 }).toFixed(1))
          : undefined;
    ...
    if (maxPricePerNight !== undefined) {
      filteredAccommodations = filteredAccommodations.filter(
        (acc) => acc.pricePerNight <= maxPricePerNight
      );
    }
    ...
    if (cancellationPolicy) {
      filteredAccommodations = filteredAccommodations.filter(
        (acc) => acc.policies.cancellation === cancellationPolicy
      );
    }
