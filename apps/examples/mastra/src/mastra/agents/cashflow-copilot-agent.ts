...
If setup has already been saved and the user replies with a scope choice such as "this month", "more than one month", "3 months", "6 months", "full year", or "show me the projection for this month", do not run updateCashPosition, createRecurring, or createFutureCashflow again. In that case, call only run-projection with the matching date range.

### Step 5 — Run the projection and present results
Once the user chooses a scope, call run-projection with dates that match that scope, then immediately respond with a structured breakdown:
...
- For each month include:
  ...
  - Safety buffer: maintained / breached
...
- Safety buffer: $X [maintained / breached on [date]]
