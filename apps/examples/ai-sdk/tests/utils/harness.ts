...
import { createTrajectory, runTrajectory, withAISdkAgent } from '@tally-evals/trajectories';
...
export interface RunCaseOptions {
  ...
  agent: {
    generate: (
      input: Prompt
    ) => Promise<{ response: { messages: readonly import('ai').ModelMessage[] } }>;
  };
...
export async function saveTallyReportToStore(args: {
  ...
  if (!runId) {
    throw new Error(
      'saveTallyReportToStore: runId is required (either pass args.runId or provide a report with runId:string)'
    );
  }

  const convRef =
    (await store.getConversation(args.conversationId)) ??
    (await store.createConversation(args.conversationId));
...
export async function runCase(options: RunCaseOptions): Promise<RunCaseResult> {
  ...
  if (RECORD_MODE) {
    ...
    const wrappedAgent = withAISdkAgent(
      agent as {
        generate: (
          input: Prompt
        ) => Promise<{ response: { messages: import('ai').ModelMessage[] } }>;
      }
    );
    const trajectoryInstance = createTrajectory({ ...trajectory, conversationId }, wrappedAgent);
 ...
 */
function extractToolResults(messages: readonly ModelMessage[]): Map<
  string,
  {
    toolCallId: string;
    toolName: string;
 ...
 */
export function assertExpectedToolCalls(step: ConversationStep, expectedToolNames: string[]): void {
  const toolCalls: Array<{ toolCallId: string; toolName: string; input: unknown }> = [];
