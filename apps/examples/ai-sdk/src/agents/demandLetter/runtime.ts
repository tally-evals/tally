...
function getEntity(session: Session): {
  ...
  switch (rep.type) {
    ...
    case 'someone_else': {
      const label = personName?.trim() || "the person you're representing";
      return { label, possessive: makePossessive(label), rep };
    }
    case 'myself': {
      const label = 'you';
  ...
  switch (question.order) {
    case 7: {
      ...
      if (otherParty !== 'the other party') {
        return base.replace(
          "the other person's name or their business name",
          otherParty,
        );
      }
      return base;
    ...
    case 9: {
      ...
      if (isMyself) return base;
      const who =
        isBusiness || isLawFirm || isSomeoneElse ? `${label}'s` : possessive;
